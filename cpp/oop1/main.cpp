...
void testDateTime() {
    ...
    */
    std::cout << "Первая дата после преобразований (все поля уменьшены на 1): ";
    first_date.printDateTime();
    std::cout << "Вторая дата после преобразований (все поля увеличены на 1): ";
    second_date.printDateTime();
    std::cout << "Третья дата без изменений: ";
    third_date->printDateTime();
    std::cout << "\n*******************************************************************************\n" << std::endl;

    // Удаление 3-го объекта вручную и проверка работоспособности счетчика объектов
    delete third_date;
    std::cout << "Объектов в счетчике после удаления одного: " << DateTime::getObjectsCounter() << std::endl;
    std::cout << "\n*******************************************************************************\n" << std::endl;

    // Удаление двух оставшихся объектов после завершения выполнения программы
    std::cout << "После завершения выполнения функции деструкторы для оставшихся объектов должны вызваться сами:\n";
}
